...
export function initWhatsAppBot() {
  ...
  const formatText = (text: string): string => {
    ...
    // Luego aplicar formato básico (solo negrita por ahora)
  ...
  const checkEasterEggs = async (text: string) => {
    ...
    const eggs: Record<string, { keywords: string[]; responses: string[] }> =
      easterEggs;

    for (const key in eggs) {
      if (eggs[key].keywords.some((k: string) => cleanText.includes(k))) {
  ...
  const handleUserReply = async (value: string, stepConfig: Step) => {
    if (stepConfig.id === 'name') {
      const bossEggs = bossEasterEggs as {
        keywords: string[];
        responses: string[];
      };
      ...
      const handleSubmit = async () => {
        ...
        if (stepConfig.type === 'email') {
          ...
          if (!emailRegex.test(val)) {
            ...
            let errorMsg =
              'Ese correo está más raro que web de 10 lucas... ¿lo revisas? 🧐';
            if (!val.includes('@')) {
